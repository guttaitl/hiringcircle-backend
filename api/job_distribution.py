...
def get_portal_config() -> Dict:
    ...
    return {
        ...
        "linkedin_organic": {
            ...
            "author_urn": os.getenv("LINKEDIN_AUTHOR_URN"),  # urn:li:organization:XXX or urn:li:person:XXX
            "api_version": "202401"
        },
...
class LinkedInOrganicPoster:
    ...
    def __init__(self, config: Dict):
        ...
        self.author_urn = config.get("author_urn")  # urn:li:organization:XXX or urn:li:person:XXX
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                ...
                error_message="LinkedIn Organic not configured. Set LINKEDIN_ACCESS_TOKEN and LINKEDIN_AUTHOR_URN."
            )
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                # Extract post ID from URN (format: urn:li:share:12345 or urn:li:ugcPost:12345)
                external_id = None
                post_url = None
                ...
                error_msg = f"LinkedIn Organic API error: {response.status_code} - {response.text}"
            ...
            error_msg = f"LinkedIn Organic posting failed: {str(e)}"
            logger.error(error_msg)
            return PortalResult(
                portal="linkedin_organic",
                status=PortalStatus.FAILED,
...
class LinkedInNativePoster:
    ...
    REQUIRES: LinkedIn Talent Partner Program approval (3-6 months).
    This posts jobs to LinkedIn's native Jobs section.
    """
    ...
    def _build_job_payload(self, job: JobPostData) -> Dict:
        ...
        if job.visa_transfer and job.visa_transfer != "No":
            sections.append(f"\n\nVisa Transfer: {job.visa_transfer}")
        ...
        # Add workplace type (remote/hybrid/onsite)
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                portal="linkedin",
                status=PortalStatus.SKIPPED,
                error_message="LinkedIn Native Jobs not configured. Set LINKEDIN_ACCESS_TOKEN and LINKEDIN_ORGANIZATION_ID."
            )
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                # Extract job ID from URN (format: urn:li:jobPosting:12345)
                ...
                error_msg = "LinkedIn Jobs API access denied. Ensure you have LinkedIn Talent Partner Program approval."
                logger.error(error_msg)
                return PortalResult(
                    portal="linkedin",
                    status=PortalStatus.FAILED,
                ...
                error_msg = f"LinkedIn Native API error: {response.status_code} - {response.text}"
                logger.error(error_msg)
                return PortalResult(
                    portal="linkedin",
                    status=PortalStatus.FAILED,
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                portal="indeed",
                status=PortalStatus.SKIPPED,
                error_message="Indeed not configured. Set INDEED_API_KEY and INDEED_SECRET."
            )
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                portal="ziprecruiter",
                status=PortalStatus.SKIPPED,
                error_message="ZipRecruiter not configured. Set ZIPRECRUITER_API_KEY."
            )
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                portal="glassdoor",
                status=PortalStatus.SKIPPED,
                error_message="Glassdoor not configured. Set GLASSDOOR_PARTNER_ID and GLASSDOOR_API_KEY."
            )
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                ...
                status=PortalStatus.SKIPPED,
                error_message="CareerBuilder not configured. Set CAREERBUILDER_API_KEY."
            )
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                portal="monster",
                status=PortalStatus.SKIPPED,
                error_message="Monster not configured. Set MONSTER_API_KEY."
            )
    ...
    def post_job(self, job: JobPostData) -> PortalResult:
        ...
        if not self.is_configured():
            return PortalResult(
                portal="simplyhired",
                status=PortalStatus.SKIPPED,
                error_message="SimplyHired not configured. Set SIMPLYHIRED_API_KEY."
            )
    ...
    def save_distribution_results(self, job_id: str, results: List[PortalResult]) -> bool:
        ...
        try:
            for result in results:
                ...
                # Implement actual DB insert here based on your schema
                # Example SQL:
...
class JobDistributor:
    ...
    def __init__(self, db_connection=None):
        ...
        self.posters = {
            # LinkedIn Organic (immediate, no approval needed)
            "linkedin_organic": LinkedInOrganicPoster(self.config["linkedin_organic"]),
            # LinkedIn Native (requires partner approval)
...
def auto_distribute_job(job: dict, db_connection=None, portals: Optional[List[str]] = None) -> Dict:
    """
    Convenience function for auto-posting a job.
    Defaults to LinkedIn Organic + other configured portals.

    Args:
        job: Job dictionary
        db_connection: Database connection
        portals: Specific portals to use (default: linkedin_organic + enabled others)

    Returns:
        Distribution results
    """
    if portals is None:
        # Default: LinkedIn Organic (immediate) + other enabled portals
        portals = ["linkedin_organic"]

        # Add other enabled portals
        config = get_portal_config()
        ...
        # Only add LinkedIn Native if explicitly enabled (requires partner approval)
...
if __name__ == "__main__":
    ...
    if ready_portals:
        ...
        print("\n⚠️  No portals fully configured. Set environment variables to enable auto-posting.")